...
bool LPT::LPT_SetStartPointFromFile(const std::string& filename)
{
  StartPointFile = filename;
  return true;
}
...
int LPT::LPT_OutputParticleData(const int& TimeStep, const double& Time, REAL_TYPE v00[4], const bool& is_last)
{
  if (!initialized) return 1;
  if (LastOutputStep == TimeStep) return 0; //このステップで出力済だった時は出力しない
  if (!is_last)
  {
    if (FileOutputInterval <= 0) return 0;                        //Intervalが設定されていない場合は粒子データのファイル出力は行なわない
    if (LastOutputStep + FileOutputInterval > TimeStep) return 0; //最後に出力してから設定されたステップ数が経過していなければファイル出力は行なわない
  }
  LastOutputStep = TimeStep;

  PMlibWrapper& PM = PMlibWrapper::GetInstance();
  PM.start("LPT_FileOutput");
  size_t NumParticles = ptrPPlib->Particles.size();
  //粒子を持っていなければ、ファイル出力せずに終了
  if (NumParticles <= 0)
  {
    PM.stop("LPT_FileOutput");
    return 0;
  }
  //粒子座標の出力
  ...
  // rworkの領域の確保/解放ができていれば、こっちのnewは失敗しないはず
  ...
  const double RefTime = OutputDimensional ? RefLength / RefVelocity : 1.0;
  //粒子が放出された時刻
  // rworkの領域の確保/解放ができていれば、こっちのnewは失敗しないはず
...
int LPT::LPT_Initialize(LPT_InitializeArgs args)
{
  ...
  //PMlibWrpperクラスの初期化
  PMlibWrapper& PM = PMlibWrapper::GetInstance();
  PM.Initialize(args.PM);

  PM.start("LPT_Initialize");
  ...
  FileOutputInterval     = args.FileOutputInterval;
  StartPointsRestartFile = args.StartPointsRestartFile;
  const double RefTime   = OutputDimensional ? RefLength / RefVelocity : 1.0;
  ...
  const int MaxDataBlockSize = ptrDM->GetInstance()->GetLargestBlockSize();
  ...
  //Comunicatorクラスの初期化
  ptrComm = new DSlib::Communicator(args.MaxRequestSize, MaxDataBlockSize, args.d_cut);
  ...
  if (MPI_Manager::GetInstance()->is_particle_proc())
  {
    ...
    if (!restart)
    {
      ...
      if (StartPointFile != "")
      {
        PPlib::ReadStartPoints(StartPointFile, &StartPoints, RefLength, RefTime);
      }
      if (MPI_Manager::GetInstance()->get_myrank_p() == 0)
      {
        PPlib::WriteStartPoints(StartPointsRestartFile, StartPoints, RefLength, RefTime);
      }
      ...
      PDMlib::PDMlib::GetInstance().SetPath(args.DirectoryPath);
      ...
      //restart実行
      LPT_LOG::GetInstance()->INFO("Read start points from file");
      PPlib::ReadStartPoints(StartPointsRestartFile, &(ptrPPlib->StartPoints), RefLength, RefTime);
      ...
      LPT_LOG::GetInstance()->INFO("Particle Data is read from file: ", NumParticles);
      delete[] ID;
      delete[] coord;
      delete[] v;
      delete[] start;
      delete[] life;
      // リスタート実行時は、指定されたタイムステップの粒子は出力済として扱う
      LastOutputStep = args.CurrentTimeStep;
    }
  }
  ...
  if (MPI_Manager::GetInstance()->is_particle_proc())
  {
    ...
    MPI_Win_create(&work_for_rerun_flag, flag_size, sizeof(int), MPI_INFO_NULL, MPI_Manager::GetInstance()->get_comm_p(), &window_for_rerun_flag);
