...
struct LPT_InitializeArgs
{
  ...
  pm_lib::PerfMonitor* PM;             //!< 計時に用いるPMlibへのポインタ
  ...
  int         FileOutputInterval;     //!< ファイル出力間隔(タイムステップ数で指定)
  std::string StartPointsRestartFile; //!< 開始点のリスタート用出力ファイル名
  std::string DirectoryPath;          //!< フィールドデータの出力先ディレクトリ

  //! Constructor
  LPT_InitializeArgs() : d_bcv(NULL),
                         d_cut(NULL),
                         RefLength(1.0),
                         RefVelocity(1.0),
                         ...
                         FileOutputInterval(-1),
                         StartPointsRestartFile("LPT_RestartFile"),
                         DirectoryPath("lpt")
  {
  }
};
