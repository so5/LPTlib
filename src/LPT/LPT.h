...
private:
  //Singletonパターンを適用
  LPT() : initialized(false), StartPointFile(""), LastOutputStep(0), NumPolling(10000), PollingRatio(0.8)
  {
  }
  LPT(const LPT& obj);
...
public:
  ...
  //! プロセス毎に異なるファイルに出力する
  int LPT_OutputParticleData(const int& TimeStep, const double& Time, float* v00, const bool& is_last = false);

  //! 粒子データを出力し、LPTの内部で保持している全ての開始点, 粒子, 流速データを破棄する
  int LPT_Post(void);

  //! 開始点のインスタンスを生成するためのインターフェース関数群
  bool LPT_SetStartPointFromFile(const std::string& filename);
...
private:
  ...
  int         FileOutputInterval;                             //!< ファイル出力間隔
  int         LastOutputStep;                                 //!< 最後にファイルを出力したタイムステップ
  std::string StartPointsRestartFile;                         //!< リスタート用の開始点情報(分割前)を保存するファイル
  ...
  std::vector< PPlib::StartPoint* > StartPoints;              //!< 開始点情報を一時的に保持するコンテナPPlibのインスタンス生成時にそちらに渡して中身は破棄される
  std::string                       StartPointFile;           //!<開始点情報が書かれたファイル
