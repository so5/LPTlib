...
public:
  ...
  bool      has_cut;       //!< データブロック内に交点が含まれるかどうかのフラグ
  //TODO ここまでを内部クラスにまとめる?
  REAL_TYPE* Data;  //!< 流速データ配列へのポインタ
  long long* d_cut; //!< カット情報配列へのポインタ

  //! コンストラクタ
  DataBlock() : Data(NULL), BlockID(-1), SubDomainID(-1), Time(-1.0), has_cut(false)
  {
    OriginCell[0] = -1;
    OriginCell[1] = -1;
    OriginCell[2] = -1;
