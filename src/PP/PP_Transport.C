...
int PP_Transport::Calc(ParticleData* Particle, const double& deltaT, const int& divT, const double& CurrentTime, const int& CurrentTimeStep)
{
  ...
  //TODO これで動いたら、Integrator/InterpolatorともにSingleton化して試す
  Interpolator* ptrIP = new Interpolator();
  Integrator*   ptrIG = new Integrator(ptrIP);
  REAL_TYPE     x_i[3];
...
#endif
    ptrIP->ConvXtoI(x_new, x_i, LoadedDataBlock->Origin, LoadedDataBlock->Pitch);

    // ルンゲ=クッタ積分
    int rkg = ptrIG->RKG(*LoadedDataBlock, dt, x_i, Particle->GetAllID()); //idはデバッグ用
    if (rkg != 0) LPT::LPT_LOG::GetInstance()->WARN("return value from RKG = ", rkg);
...
#pragma forceinline recursive
#endif
      ptrIP->InterpolateData(*LoadedDataBlock, x_i, v, "");
    } else {
      counter++;
