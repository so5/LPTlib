...
int Circle::CalcSumStartPoints()
{
  ...
  //円の中心の座標はtheta_max=2*M_PIの領域のみ含める
  if (theta_max == 2 * M_PI)
  {
    ++tmpSumStartPoints;
  }
...
void Circle::Divider(std::vector< StartPoint* >* StartPoints, const int& MaxNumStartPoints)
{
  ...
  //中心角を、最外周の円上の点の数を基準に分割する。
  int NumPointsOnOutermostCircle = Iend * a;

  //余りオブジェクトの中心角は最外周の円状にある開始点数をもとに分割する
  //まず、余りオブジェクトを作成して含まれる開始点数が少ない場合は
  //すこしづつ中心角を広げる
  //逆に余りオブジェクトに余計に開始点が含まれている場合はそのままにする
  double ReminderCentralAngle = 0.0;
  if (NumReminder != 0)
  {
    ReminderCentralAngle = (double)(NumPointsOnOutermostCircle % NumParts) / (double)NumPointsOnOutermostCircle * 2 * M_PI;
    Circle* NewCircle    = CircleFactory(this->Coord1, this->SumStartPoints, this->Radius, this->NormalVector, this->StartTime, this->ReleaseTime, this->TimeSpan, this->ParticleLifeTime);
    NewCircle->theta_min = 2 * M_PI - ReminderCentralAngle;
    int i                = 1;
    while (NumReminder > NewCircle->CalcSumStartPoints())
    {
      ReminderCentralAngle = (double)(NumPointsOnOutermostCircle % NumParts + i) / (double)NumPointsOnOutermostCircle * 2 * M_PI;
      NewCircle->theta_min = 2 * M_PI - ReminderCentralAngle;
      i++;
    }
