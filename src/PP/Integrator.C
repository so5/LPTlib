...
namespace PPlib
{
bool Integrator::GetIntegrand(const DSlib::DataBlock& DataBlock, const REAL_TYPE x_i[3], REAL_TYPE func[3], const std::string& id)
{
  REAL_TYPE v[3];
  ptrIP->InterpolateData(DataBlock, x_i, v, id);
...
int Integrator::RKG(const DSlib::DataBlock& DataBlock, const double t_step, REAL_TYPE x_i[3], const std::string& id)
{
...
#endif
    GetIntegrand(DataBlock, x_i, func, id);
    for (int n = 0; n < 3; n++)
    {
      k = t_step * func[n];
      r = cx[l] * (k - ck[l] * q[n]);
